...
export async function getElevationProfile(routeCoordinates: [number, number][], totalDistanceKm: number): Promise<ElevationPoint[]> {
  ...
  if (routeCoordinates && routeCoordinates.length > 0) {
    ...
    if (apiKey) {
      try {
        ...
        const otRes = await axios.get(`https://portal.opentopography.org/API/globaldem?demtype=SRTMGL1&locations=${locations}&outputFormat=JSON&API_Key=${apiKey}`, {
          timeout: 4000
        });
        if (otRes.data && Array.isArray(otRes.data.elevations)) {
          const count = otRes.data.elevations.length;
          return otRes.data.elevations.map((elev: number, idx: number) => ({
    ...
    try {
      const omRes = await axios.get(`https://api.open-meteo.com/v1/elevation?latitude=${lats}&longitude=${lngs}`, {
        timeout: 4000
      });
      if (omRes.data && Array.isArray(omRes.data.elevation)) {
        const count = omRes.data.elevation.length;
        return omRes.data.elevation.map((elev: number, idx: number) => ({
          distanceKm: Math.round((idx / Math.max(count - 1, 1)) * totalDistanceKm),
          elevationM: Math.round(elev)
        }));
      }
