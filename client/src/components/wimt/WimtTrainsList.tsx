          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative">
              ...
              >
                ...
                <option value="Tomorrow" className="text-slate-900">📅 Tomorrow, 2 Sep</option>
              </select>
          ...
          return (
            <div
              ...
              className={`p-4 hover:bg-blue-50/50 cursor-pointer transition-colors ${
                isDeparted ? 'bg-slate-50 opacity-60' : 'bg-white'
              }`}
            >
              
              {/* Top Line: Train Number Badge + Timings + Running Days */}
              <div className="flex items-center justify-between">
                
                <div className="flex items-center gap-2">
                  <span className={`px-2 py-0.5 font-bold text-xs rounded text-white ${
                    isDeparted ? 'bg-slate-400' : 'bg-[#0288D1]'
                  }`}>
                    {t.trainNumber}
                  </span>

                  <div className="flex items-center gap-1.5 text-sm font-extrabold text-slate-900">
                    <span>{t.departureTime || '6:00 AM'}</span>
              ...
              <div className="flex items-center justify-between mt-1.5">
                ...
                <div className="flex items-center gap-1 shrink-0">
                  <span className={`px-1.5 py-0.5 text-[10px] font-bold rounded border ${
                    isDeparted ? 'bg-slate-200 text-slate-500 border-slate-300' : 'bg-emerald-100 text-emerald-800 border-emerald-300'
                  }`}>
                    GN
                  </span>
                  <span className={`px-1.5 py-0.5 text-[10px] font-bold rounded border ${
